...
public class SavingAccount extends Account {
    ...
    public SavingAccount(int initialBalance, int minBalance, int maxBalance, int rate) {
        ...
        if (initialBalance < 0 | initialBalance < minBalance | initialBalance > maxBalance) {
            throw new IllegalArgumentException(
         "Начальный баланс не может быть отрицательным, меньше заданного лимита и больше значения Max"
            );
        }
     ...
     * @return true если операция прошла успешно, false иначе. если начальный баланс меньше суммы покупки то false
     */
    @Override
